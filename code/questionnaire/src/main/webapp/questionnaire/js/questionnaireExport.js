function exportXls(){
			...
			var uri = 'data:application/vnd.ms-excel;base64,'
				  , template = '<head><!--[if gte mso 9]><?xml version="1.0" encoding="UTF-8" standalone="yes"?><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head>'+
				  '<body><table style="vnd.ms-excel.numberformat:@">{table}</table></body>'
				  , base64 = function (s) { return window.btoa(unescape(encodeURIComponent(s))); }
				  , format = function (s, c) { return s.replace(/{(\w+)}/g, function (m, p) { return c[p]; }); };
				var ctx = { worksheet: name || 'Worksheet', table: table };
		    download((uri + base64(format(template, ctx))), (name+".xls"), "text/plain");
};
function exportDoc(){
	...
	var uri = 'data:application/vnd.ms-word;base64,',
		...
		'<body><div>{content}</div></body></html>'
		  ...
		  download((uri + base64(format(template, ctx))), "a.doc","text/plain");
};